...
final class ArticleAIViewModel: ObservableObject {
    ...
    private func recomputeCTAIfPossible() {
        // Complimentary quota is tracked server-side by deviceToken and returned as `guestRemaining`,
        // even after SIWA. So always use `guestRemaining` for the free bucket.
    ...
    private func decrementComplimentaryIfKnown() {
        // Complimentary quota is one bucket tracked by server/deviceToken.
...
//    }
    ...
    @MainActor
    func refreshGuestQuota(force: Bool = false) async {
        // Complimentary quota is tracked on the server by deviceToken (even after SIWA).
        // So we refresh in both signed-in and signed-out states.

        if !force, quota.guestRemaining != nil { return }

        do {
            let remaining = try await service.fetchQuota()
            // Preserve the current signed-in flag.
            ...
            #endif
            // Leave quota unchanged on failure.
        }
    }
    ...
    func generate(for article: Article) async {
        ...
        generateTask = Task { [weak self] in
            ...
            do {
                ...
                // Optimistically decrement the displayed remaining count to keep the UI responsive.
                // Authoritative remaining count is tracked on the server by deviceToken.
                self.decrementComplimentaryIfKnown()

                // Re-sync from server (also ensures AIUsage exists/updates as expected).
                await self.refreshGuestQuota(force: true)
                ...
                if let payload = self.decodeAIInsightServerPayload(from: error) {
                    ...
                    if payload.requiresSignIn == true {
                        // SIWA required before complimentary insights can be used.
                        self.state = .idle
                    ...
                    if payload.requiresSubscription == true {
                        // Complimentary quota exhausted; subscription required.
                        self.state = .idle
