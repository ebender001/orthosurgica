...
final class MeshCatalogManager: ObservableObject {
    ...
    private enum Cache {
        private static let namespace = AppBranding.meshCatalogCacheNamespace
        private static let cachedFileName = "mesh_catalog_\(namespace).json"
        private static let cachedVersionKey = "mesh_catalog_cached_version_\(namespace)"
    ...
    func loadRemote() async {
        ...
        // 2) Fall back to the bundled specialty catalog if no cached catalog exists yet.
        if catalog == nil {
            do {
                catalog = try MeshCatalogLoader.load(name: AppBranding.bundledMeshCatalogName)
                status = "Loaded bundled catalog"
            } catch {
                self.error = error.localizedDescription
            }
        }

        // 3) Always fetch remote on launch (version + data)
        do {
...
//            #endif
            ...
            // Remote failed; keep cached or bundled data if we have it.
            if catalog != nil {
                if status == "Loaded bundled catalog" {
                    status = "Offline (using bundled catalog)"
                } else {
