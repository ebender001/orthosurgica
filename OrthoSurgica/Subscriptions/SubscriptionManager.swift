...
struct DeleteMyAccountCloud: ParseCloudable {
    var functionJobName: String = "deleteMyAccount"
    typealias ReturnType = EmptyCloudResponse
}
...
final class SubscriptionManager: ObservableObject {
    ...
    let productIDs: Set<String> = [
        "com.benderapps.orthosurgica.ai.monthly",
        "com.benderapps.orthosurgica.ai.annual"
    ]
    ...
    func deleteAccount() async -> Bool {
        ...
        guard User.current != nil else {
            lastErrorMessage = "No signed-in account found."
            return false
        }

        do {
            let payload = DeleteMyAccountCloud()
            _ = try await payload.runFunction()

            try await User.logout()

            entitlement = .inactive
            UserDefaults.standard.removeObject(forKey: subscriptionSyncKey)

            return true
        } catch {
            lastErrorMessage = "Unable to delete account right now. Please try again."
            return false
        }
    }
...
enum AIInsightCTAState: Equatable {
    ...
    var helperText: String? {
        ...
        case .freeRemaining(let remaining):
            ...
            return nil
        case .needsSignIn:
            return "Sign in to enable your 3 complimentary insights."
        case .needsSubscription:
...
extension AIInsightCTAState {
    ...
    static func resolve(
        ...
        // Not signed in: require SIWA before any complimentary insights can be used.
        // (We still fetch/show remaining elsewhere, but CTA is sign-in.)
        if !isSignedIn {
            return .needsSignIn
        }

        // Signed in (no subscription): allow complimentary insights until exhausted.
        if let remaining = guestFreeRemaining {
            return remaining > 0 ? .freeRemaining(remaining: remaining) : .needsSubscription
        }

        // Unknown quota (e.g. before server fetch finishes).
        return .loading
    }
}
