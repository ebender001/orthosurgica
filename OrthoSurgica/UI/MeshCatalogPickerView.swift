...
struct MeshCatalogPickerView: View {
    ...
    private let selectTopicTip = SelectTopicToCreateSearchTip()
        ...
        Form {
            ...
            Section("Select Topic Category") {
                if let loadError = meshManager.error {
                    ...
                    Picker(selection: $majorHeading) {
                        ForEach(catalog.majorHeadingsSorted, id: \.self) { Text($0) }
                    } label: {
                        Text("Major heading")
                            .foregroundStyle(topicAccentColor)
                    }

                    Picker(selection: $subspecialty) {
            ...
            if !catalog.isEmpty, meshManager.error == nil {
                ...
                Section("Tap a topic to build your search") {
                    TipView(selectTopicTip)

                    if terms.isEmpty {
                        ContentUnavailableView(
                            "No topics found",
                            systemImage: "magnifyingglass",
                            description: Text("Choose a different topic, then tap a topic below to add it and generate search text.")
                        )
                    } else {
                        ...
                        ForEach(terms, id: \.term) { t in
                            ...
                            Button {
                                ...
                                HStack(alignment: .firstTextBaseline) {
                                    ...
                                    if isAlreadyAdded {
                                        paletteIcon("checkmark", tint: topicAccentColor, size: 24, symbolSize: 12)
                                            .opacity(0.9)
                                    } else {
                                        paletteIcon("plus", tint: topicAccentColor, size: 24, symbolSize: 12)
                                    }
                                }
                                .padding(.vertical, 6)
        ...
        .toolbar {
            ToolbarItem(placement: .topBarTrailing) {
                Button {
                    ...
                    paletteIcon("lightbulb", tint: topicAccentColor, size: 30, symbolSize: 14)
                }
